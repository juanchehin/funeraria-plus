...
<!--[if gt IE 8]><!--> <html class="no-js"> <!--<![endif]-->
	<head>
		...
		<title>Morgue Management | <?php echo (isset($x->TableTitle) ? $x->TableTitle : ''); ?></title>
	...
	<body>
		<div class="container theme-cerulean theme-compact">
			...
			<?php 
				...
				$Embedded = $Embedded ?? []; // Si es null, lo convierte en un array vacío
			...
			<?php 
				...
				$Embedded = $_REQUEST['Embedded'] ?? []; // Si no existe, asigna un array vacío

				// Aquí también usas el operador ?? para evitar el acceso a un índice no definido
				if(empty($Embedded)) {
					$notification_margin = ($_REQUEST['Embedded'] ?? false ? '15px 0px' : '-15px 0 -45px');
				}
